...
model = dict(
    ...
    backbone=dict(
        ...
        depth=53,
        out_indices=(3, 4, 5),
        init_cfg=dict(type='Pretrained', checkpoint='work_dirs/yolov3_d53_8xb8-ms-608-273e_coco/epoch_4.pth')),
    neck=dict(
        type='YOLOV3Neck',
        num_scales=3,
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations', with_bbox=True),
    dict(
        type='YOLOv3ResizePad',
        img_scale=(608, 608),  # 需要调整为 YOLO 默认的 (608, 608) 或其它适当的尺寸
        keep_ratio=True,
        pad_mode='default'
    ),
    dict(prob=0.5, type='RandomFlip'),
...
test_pipeline = [
    ...
    dict(
        type='YOLOv3ResizePad',
        img_scale=(608, 608),  # 调整为 YOLO 默认的 (608, 608) 或其它适当的尺寸
        keep_ratio=True,
        pad_mode='default'
    ),
    dict(type='LoadAnnotations', with_bbox=True),
...
val_evaluator=dict(
    eval_mode='11points', metric='mAP', type='VOCMetric')
test_evaluator=dict(
    eval_mode='11points', metric='mAP', type='VOCMetric')